...
x1 <- plot(n_hom_gb_emm) + geom_boxplot() + theme_bw() + xlab("EMMEAN of homozygous LOF variants") + 
  ylab("Breed") +scale_x_continuous(labels=comma) + 
  theme(panel.grid = element_blank(), panel.border = element_blank(), 
        axis.line.x = element_line(), axis.line.y = element_line(), 
        axis.text.x = element_text(), axis.text = element_text(size=10), 
...
data1 <- gb_br[c(1,3,4)]
dfm <- melt(data1, id.vars = "Sample")
formatter <- function(...){
  function(x) format(round(x, 1), ...)
}
gb_hist <- ggplot(gb_br, aes(x = total,fill = breed,color = breed)) +
...
lof1 <- lof[c(1,9,15)]
dfm <- melt(lof1, id.vars = "group")

lof_bar <- ggplot(dfm, aes(x = value,fill = variable,color = variable)) +
  geom_bar(position = "dodge2", width = 0.8, alpha = 1) + ylab("Frequency") + xlab("Number of LOF variants") + 
...
dfm <- melt(se_ann, id.vars = "consequence", na.rm = TRUE)
dfm$value <- as.numeric(dfm$value)

x = ggplot(dfm, aes(x = consequence, y = value, fill = variable, color = variable)) +
  geom_bar(stat = "identity", position = "dodge2", width = 0.8, alpha = 1) + ylab("Frequency") + xlab("Number of LOF variants") + 
