...
rare_breed = {}
with open("../rare_breed_common_po/rare_breed_common_pop_shared_variants.txt", "r") as input_file:
    for line in input_file:
        line = line.rstrip("\n").split("\t")
        ab = line[0] + ":" + line[1]
        rare_breed[ab] = line[2] + ":" + line[3] + ":" + line[4] + ":" + line[5]

with open("../rare_breed_common_po/rare_breed_common_pop_snpeff_variants.txt","r") as input_file, open("../rare_breed_common_po/rare_breed_common_pop_snpeff_info.txt", "w") as output_file:
    print("chromosome\tposition\tpop_AC\tpop_AF\tconsequence\timpact\tgene\tcoding\tprotein\tnuclear\tbreed_rare\tbreed_AC\tMultiallelic", file = output_file)
    for line in input_file:
        line = line.rstrip("\n").split("\t")
        chrom_pos = line[0] +":" + line[1]
        ab = line[7].split(";")
