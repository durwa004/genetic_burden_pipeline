...
if __name__ == '__main__':
    ...
    for filename in os.listdir(data + "/../"):
        if filename.endswith(".vcf.gz"):
            breed = filename.split("_")
            breed = breed[2].split(".vcf")
            breed = breed[0]
            count_rare = 0
            count_common = 0
            count_other = 0
            with gzip.open(data + "/../" + filename, "rt") as input_file, open(data + f"/{breed}_rare.vcf", "w") as rare, open(data + f"/{breed}_common.vcf", "w") as common:
